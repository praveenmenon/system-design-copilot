...
export function jobsScene(): DiagramData {
    const E:any[]=[];
    const client=P(L.client,0); R(E,"client",client,"Client");
    const gw=P(L.gateway,0); R(E,"gw",gw,"Jobs API");
    const q=P(L.async,0); R(E,"queue",q,"Job Queue");
    const workers=P(L.async,1); R(E,"workers",workers,"Worker Pool");
    const db=P(L.data,0); R(E,"db",db,"Jobs DB");
  
    A(E, client, gw, "POST /jobs");
    A(E, gw, q, "Enqueue");
    A(E, q, workers, "Consume");
    A(E, workers, db, "Update status");
