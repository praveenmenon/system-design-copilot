export type DiagramData = {
  ...
  export function workflowScene(): DiagramData {
    const E:any[]=[];
    const client=P(L.client,0); R(E,"client",client,"Client");
    const gw=P(L.gateway,0); R(E,"gw",gw,"API Gateway");
    const orc=P(L.service,0); R(E,"orc",orc,"Orchestrator");
    const q=P(L.async,0); R(E,"queue",q,"Task Queue");
    const worker=P(L.async,1); R(E,"worker",worker,"Task Worker");
    const db=P(L.data,0); R(E,"db",db,"State Store");
  
    A(E, client, gw, "POST /workflows");
    A(E, gw, orc, "Start");
    A(E, orc, q, "Enqueue steps");
    A(E, q, worker, "Consume");
    A(E, worker, db, "Update step");
    A(E, orc, db, "Update state");
