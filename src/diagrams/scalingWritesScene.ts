export type DiagramData = {
  ...
  export function scalingWritesScene(): DiagramData {
    const E:any[]=[];
    const prod=P(L.client,0); R(E,"producer",prod,"Producer");
    const gw=P(L.gateway,0); R(E,"ingest",gw,"Ingest API");
    const buf=P(L.async,0); R(E,"buffer",buf,"Buffer/Queue");
    const router=P(L.service,0); R(E,"router",router,"Shard Router");
    const s1=P(L.data,0); R(E,"s1",s1,"Shard A");
    const s2=P(L.data,1); R(E,"s2",s2,"Shard B");
  
    A(E, prod, gw, "POST /events");
    A(E, gw, buf, "Buffer");
    A(E, buf, router, "Consume");
    A(E, router, s1, "Write (key%2==0)");
    A(E, router, s2, "Write (key%2==1)");
  ...
  export default scalingWritesScene;
